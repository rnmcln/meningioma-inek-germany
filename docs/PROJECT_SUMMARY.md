...
## 7. Note on the non-resection group

"Without a resection code" is a coding category, not a clinical one, and is not equivalent to "non-surgical". Because the resection definition is restricted to the two attributable excision codes (5-015.3, 5-015.4), episodes without that code include genuinely operated cases whose principal procedure was coded elsewhere (intracerebral excision 5-015.0/.1, skull-base resections 5-016.x, cranial-nerve tumour resection 5-017.1), pure diagnostic biopsies (coded in the diagnostic OPS chapter, e.g. 1-510/1-511; uncommon for meningioma), a small amount of inpatient radiotherapy, and non-operative admissions for symptom or complication management or comorbidity. The marginal aggregate queries do not allow this group to be decomposed quantitatively.

## 8. Skull-base sensitivity (2024)

Because the primary resection definition is restricted to the convexity-focused excision codes 5-015.3 and 5-015.4, skull-base meningioma resections coded as 5-016.40 to .43 (skull-base tumour with dura, by cranial fossa) fall outside it. A sensitivity query of the InEK DatenBrowser (principal D32.0, 2024) found 115 episodes with a 5-016.40 to .43 code, of which 81 carried neither 5-015.3 nor 5-015.4. Adding these codes raises the resection-associated count from 5,646 to 5,727 and the resection share from 67.5% to 68.5%, leaving 2,634 episodes without any of these resection codes. The values are in `phase1_extraction/data/inek_D32.0_skullbase_sensitivity_2024.csv`. The 5-016 codes are skull-base site codes and are not meningioma-specific, so this is a sensitivity analysis rather than the primary definition.
